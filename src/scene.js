...
export function createScene(container, cells, accent, reduced) {
  ...
  const sweep = new THREE.DirectionalLight('#fff4d8', 0); sweep.position.set(-6,3,7); scene.add(sweep);
  ...
  const hangingMaterial = new THREE.MeshPhysicalMaterial({ roughness: .9, side: THREE.DoubleSide });
  ...
  const flat = new THREE.Mesh(new THREE.PlaneGeometry(6, 8, 24, 32), flatMaterial); board.add(flat);
  ...
  panel(46, 10, 7, 1, -9, 0, -Math.PI / 2, plaster);
  const floor = panel(14, 46, 0, -4, -9, -Math.PI / 2, 0, floorMaterial);
  ...
  for (let i=0;i<6;i++) {
    ...
    const mesh = new THREE.Mesh(g, hangingMaterial.clone());
    mesh.position.set(i%2 ? 4.3 : -4.3, .5, -2 - Math.floor(i/2)*9);
    mesh.rotation.y = i%2 ? -.35 : .35;
    mesh.castShadow = mesh.receiveShadow = true;
    mesh.userData.destination=mesh.position.clone();mesh.userData.angle=mesh.rotation.y;
    room.add(mesh); screens.push(mesh);
    const rail = new THREE.Mesh(new THREE.BoxGeometry(3.8,.065,.065),brass);
    rail.position.copy(mesh.position);rail.position.y=4.1;rail.rotation.y=mesh.rotation.y;room.add(rail);rails.push(rail);
  }
  const frames = [];
  for (let i = 0; i < 8; i++) {
  ...
  let editEnergy=0,clothAmount=0,metalAmount=0;
  const ease=(a,b,v)=>THREE.MathUtils.smootherstep(v,a,b);
  ...
  function update(newCells, newAccent, nextMaterial = material) {
    ...
    for(const mesh of screens){
      ...
      mat.map?.dispose();mat.map=hangingTexture.clone();mat.map.needsUpdate=true;
      mat.metalness=material==='metal'?.96:0;mat.roughness=material==='metal'?.19:material==='textile'?1:.78;
      mat.sheen=material==='textile'?1:0;mat.sheenColor.set('#eedcc0');mat.sheenRoughness=.85;
      mat.anisotropy=material==='metal'?.7:0;
      mat.bumpMap=material==='textile'?weaveTexture:null;mat.bumpScale=material==='textile'?.065:0;
      mat.envMapIntensity=material==='metal'?1.65:.85;mat.needsUpdate=true;
    }
  ...
  renderer.setAnimationLoop(time => {
    ...
    immersion = reduced ? target : THREE.MathUtils.clamp(immersion+(target===1?dt/2.7:-dt/1.1),0,1);
    editEnergy=THREE.MathUtils.damp(editEnergy,0,8,dt);
    clothAmount=reduced?(material==='textile'?1:0):THREE.MathUtils.damp(clothAmount,material==='textile'?1:0,5,dt);
    metalAmount=reduced?(material==='metal'?1:0):THREE.MathUtils.damp(metalAmount,material==='metal'?1:0,5,dt);
    smoothed.x = THREE.MathUtils.damp(smoothed.x, pointer.x, 3, dt);
    smoothed.y = THREE.MathUtils.damp(smoothed.y, pointer.y, 3, dt);
    const p = ease(0,1,immersion);
    const mobile = camera.aspect < .8;
    const x = (phase === 'intro' || phase === 'end') ? (mobile ? -.2 : -3.2) : (mobile ? 0 : innerWidth<=1100 ? -4.4 : -2.3);
    ...
    board.scale.setScalar((phase==='intro'?1.17:1.12) + editEnergy*.035);
    board.visible = immersion < .015;
    room.visible = immersion >= .015;
    room.scale.setScalar(1);
    const architecture=ease(.28,.9,immersion);
    floor.rotation.x=-Math.PI/2*ease(.16,.76,immersion);
    floor.position.set(0,THREE.MathUtils.lerp(0,-4,architecture),THREE.MathUtils.lerp(-.2,-9,architecture));
    floor.scale.set(THREE.MathUtils.lerp(6/14,1,architecture),THREE.MathUtils.lerp(8/46,1,architecture),1);
    frames.forEach((f,i)=>{const q=ease(.28+i*.025,.7+i*.025,immersion);f.scale.set(1,Math.max(.001,q),1);});
    relief.scale.setScalar(Math.max(.001,ease(.5,.96,immersion)));
    sweep.intensity=metalAmount*3.2;
    sweep.position.set(reduced?-4:Math.sin(time*.00038)*9,4,reduced?5:7+Math.cos(time*.00038)*3);
    ...
    for(const [i,m] of screens.entries()) {
      ...
      const unfold=ease(.025+i*.035,.65+i*.045,immersion);
      const dest=m.userData.destination;
      m.position.set(THREE.MathUtils.lerp(1.6+(i-2.5)*1.12,dest.x,unfold),THREE.MathUtils.lerp(0,dest.y,unfold),THREE.MathUtils.lerp(0,dest.z,unfold));
      m.rotation.set(0,m.userData.angle*unfold+Math.sin(unfold*Math.PI)*(i%2?-.55:.55),-.08*(1-unfold));
      m.scale.set(THREE.MathUtils.lerp(1.12/3.5,1,unfold),THREE.MathUtils.lerp(8.96/7,1,unfold),1);
      m.material.map.repeat.set(THREE.MathUtils.lerp(scale/6,Math.max(1,scale*.5),unfold),scale);
      m.material.map.offset.x=(1-unfold)*i*scale/6;
      rails[i].visible=immersion>.65;rails[i].scale.x=ease(.65,.95,immersion);
      const positions=m.geometry.attributes.position,rest=m.geometry.userData.restPositions;
      for(let j=0;j<positions.count;j++) {
        const x=rest[j*3],y=rest[j*3+1],fall=(3.5-y)/7;
        const breeze=reduced?0:Math.sin(time*.0009+i+x*.65)*.26*fall;
        const pleat=Math.sin(x*5.3+i+breeze)*.58*(.45+fall);
        const paperCurl=(x*x*.075+Math.pow(fall,5)*.32)*(1-clothAmount)*(1-metalAmount);
        positions.setZ(j,(pleat*clothAmount+paperCurl+breeze*clothAmount)*unfold);
        positions.setY(j,y+Math.cos(x*5.3+i)*.13*fall*fall*clothAmount*unfold);
      }
  ...
  return {
    update,
    setPhase(value) { phase = value; target = phase === 'room' ? 1 : 0; if(reduced)immersion=target; },
    pulse(){if(!reduced)editEnergy=1;},
    ...
    isFallback: false,
    dispose() { renderer.setAnimationLoop(null); ro.disconnect();screens.forEach(m=>{m.material.map?.dispose();m.material.dispose();}); texture.dispose();floorTexture.dispose();hangingTexture.dispose();weaveTexture.dispose(); env.dispose(); scene.traverse(o => { o.geometry?.dispose(); }); [flatMaterial,wallMaterial,floorMaterial,hangingMaterial,plaster,ceiling,brass,vermilion].forEach(m=>m.dispose()); renderer.dispose(); }
  };
}
